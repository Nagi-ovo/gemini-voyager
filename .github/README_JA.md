...
<p align="center">
  <a href="https://nagi.fun/gemini-voyager/">📖 ドキュメント</a> • 
...
### 🍌 NanoBanana オプション
**Gemini 画像の透かしをロスレス除去。**
逆アルファブレンドアルゴリズムを使用し、AI 生成画像 (NanoBanana) 上の目に見える「Gemini」透かしを自動的に除去します。
- **品質劣化ゼロ**：元のピクセルを完全に復元し、クリーンな画像を取得します。
- **設定は一度だけ**：設定パネルで有効にするだけで、あとは全自動で動作します。
- **直接ダウンロード**：標準のダウンロードボタンを 🍌 ボタンに置き換えます。クリックするだけで、100% 透かしなしの処理済みバージョンを直接ダウンロードできます。
- **プライバシー保護**：すべての処理はブラウザのローカルで 100% 完結します。

> [!NOTE]
> 本機能は、[journey-ad](https://github.com/journey-ad) 氏による [gemini-watermark-remover](https://github.com/journey-ad/gemini-watermark-remover) アルゴリズムに基づいており、その核心ロジックは [allenk](https://github.com/allenk) 氏の [C++ オリジナル版](https://github.com/allenk/GeminiWatermarkTool) に由来します。🧡
...
**あなたのデータは、あなたのもの。**
会話を必要な形式で保存しましょう。
- **多彩なフォーマット**：構造化された JSON、または印刷や閲覧に適した Markdown/PDF。
- **クリーン**：インターフェース上のノイズを自動的に取り除き、核心となるコンテンツのみを保持します。
...
- **ワンクリック削除**：削除ボタンをクリックして、選択した会話を一括でクリーンアップします。
