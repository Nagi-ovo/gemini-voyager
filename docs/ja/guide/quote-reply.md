...
Gemini Voyager は便利な「引用返信」機能を提供し、特定のコンテキストに対する返信をより正確かつ効率的にします。

## 機能紹介

日常的な対話の中で、AI の出力の特定部分についてさらに質問したり、反論したりしたい場面がよくあります。従来の方法では、その部分をコピーし、入力欄で手動で `> ` 記号を打つ必要があり、非常に面倒でした。

Voyager はこのフローをシンプルにします：

1. **選択して引用**：会話ページ（あなたの質問でも Gemini の回答でも）で、任意のテキストをマウスで選択します。
...
- **標準フォーマット**：一般的な Markdown 構文を使用するため、Gemini はこの引用構造を完璧に理解し、より的確な応答を返すことができます。
