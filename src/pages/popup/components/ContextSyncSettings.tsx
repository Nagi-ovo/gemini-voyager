...
export function ContextSyncSettings() {
  ...
  const [statusMessage, setStatusMessage] = useState<{ text: string; kind: 'ok' | 'err' | 'info' } | null>(null);
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      if (tab.url && !tab.url.includes('gemini.google.com') && !tab.url.includes('chatgpt.com') && !tab.url.includes('claude.ai')) {
      ...
      <CardContent className="space-y-4 p-0">
        ...
        <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-medium">{isOnline ? t('ideOnline') : t('ideOffline')}</span>
            </div>
        ...
        {statusMessage && (
          <p className={`text-xs text-center ${
            statusMessage.kind === 'ok' ? 'text-green-600' : 
            statusMessage.kind === 'err' ? 'text-destructive' : 'text-muted-foreground'
          }`}>
            {statusMessage.text}
          </p>
        )}
        ...
        {!isOnline && (
             <p className="text-xs text-muted-foreground text-center">{t('checkServer')}</p>
        )}
      </CardContent>
    </Card>
  );
