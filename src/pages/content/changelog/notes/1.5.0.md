...
- **Remote Voyager notices**: Voyager can now surface maintainer notices between releases from a public data feed, using system notifications when allowed and an in-page fallback otherwise.
...
- **Voyager 远程公告**：维护者现在可以通过公开数据源在发版间隔中提示重要消息；允许时使用系统通知，否则使用页面内提示。
...
- **Voyager 遠端公告**：維護者現在可以透過公開資料來源，在發版間隔中提示重要訊息；允許時使用系統通知，否則使用頁面內提示。
...
- **Voyager のリモート通知**：公開データフィードから、リリースの合間にもメンテナーからのお知らせを表示できます。許可済みならシステム通知、そうでなければページ内通知を使います。
...
- **Annonces Voyager à distance** : Voyager peut désormais afficher des messages de maintenance entre deux versions depuis un flux de données public, via notifications système si autorisées ou bannière intégrée sinon.
...
- **Avisos remotos de Voyager**: Voyager ahora puede mostrar mensajes del equipo entre versiones desde un feed público de datos, con notificaciones del sistema si están permitidas o un aviso dentro de la página si no.
...
- **Avisos remotos do Voyager**: O Voyager agora pode mostrar mensagens de manutenção entre versões a partir de um feed público de dados, usando notificações do sistema quando permitidas ou um aviso na página como alternativa.
...
- **إشعارات Voyager البعيدة**: يستطيع Voyager الآن عرض رسائل من المشرفين بين الإصدارات من موجز بيانات عام، عبر إشعارات النظام عند السماح بها أو تنبيه داخل الصفحة عند عدم توفرها.
...
- **Voyager 원격 공지**: Voyager가 이제 공개 데이터 피드에서 릴리스 사이의 관리자 공지를 표시할 수 있으며, 허용된 경우 시스템 알림을 쓰고 그렇지 않으면 페이지 안 알림을 사용합니다.
...
- **Удалённые уведомления Voyager**: Voyager теперь может показывать сообщения сопровождающих между релизами из публичного потока данных, через системные уведомления при разрешении или через уведомление на странице.
