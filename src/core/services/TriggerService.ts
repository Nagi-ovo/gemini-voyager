...
export class TriggerService {
  ...
  private setupSendInterceptors() {
    ...
    document.addEventListener(
      ...
      (e) => {
        ...
        const isSendKey = this.ctrlEnterSend ? e.ctrlKey || e.metaKey : !e.ctrlKey;
        if (!isSendKey) return;
  ...
  private checkAndTriggerCategorization(el: HTMLElement) {
    console.warn('[GV-Trigger] checkAndTriggerCategorization called');
    if (this.isTemporaryChat()) {
      console.warn('[GV-Trigger] Skipped: temporary chat');
      return;
    }
    ...
    if (!trimmed) {
      console.warn('[GV-Trigger] Input empty, aborting');
      return;
    }

    if (!this.isEnabled) {
      console.warn('[GV-Trigger] Feature is disabled, aborting check');
      return;
    }
    ...
    console.warn(
      '[GV-Trigger] rawText:',
      JSON.stringify(rawText.substring(0, 50)),
      'matchedPrefix:',
      matchedMainPrefix,
      'mode:',
      this.triggerMode,
    ...
    if (this.indexRouting) {
      ...
      console.warn('[GV-Trigger] Index routing result:', routingResult);
  ...
  private tryIndexRouting(trimmedText: string, matchedMainPrefix: string | undefined): boolean {
    ...
    console.warn(
      '[GV-Trigger] tryIndexRouting matched! pathParts:',
      pathParts,
      'remaining:',
      remainingPrompt,
    );
